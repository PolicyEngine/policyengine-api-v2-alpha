...
def run_simulation(simulation_id: str, session: Session):
    ...
    with logfire.span("run_simulation", simulation_id=simulation_id):
        with logfire.span("load_simulation_metadata"):
            ...
            if not model_version:
                raise ValueError(
                    f"Model version {simulation.tax_benefit_model_version_id} not found"
                )
        ...
        # Import policyengine
        with logfire.span("import_policyengine", model=tax_benefit_model.name):
        ...
        with logfire.span(
            "load_policy_and_dynamic",
            has_policy=simulation.policy_id is not None,
            has_dynamic=simulation.dynamic_id is not None,
        ):
        ...
        with logfire.span("create_pe_dataset", name=dataset.name, year=dataset.year):
        ...
        with logfire.span(
            "create_pe_simulation",
            has_policy=policy is not None,
            has_dynamic=dynamic is not None,
        ):
            console.print(
                f"  Creating simulation with policy={policy is not None}, dynamic={dynamic is not None}"
            )
            PESimulation(
                dataset=pe_dataset,
        ...
        with logfire.span("mark_simulation_complete"):
...
def run_report(report_id: str, session: Session):
    ...
    with logfire.span("run_report", report_id=report_id):
        with logfire.span("load_report_metadata"):
        ...
        with logfire.span("load_policies_and_dynamics"):
            baseline_policy = _get_pe_policy(
                baseline_sim.policy_id, pe_model_version, session
            )
            reform_policy = _get_pe_policy(
                reform_sim.policy_id, pe_model_version, session
            )
            baseline_dynamic = _get_pe_dynamic(
                baseline_sim.dynamic_id, pe_model_version, session
            )
            reform_dynamic = _get_pe_dynamic(
        ...
        with logfire.span("create_baseline_simulation"):
        ...
        with logfire.span("create_reform_simulation"):
        ...
        with logfire.span("calculate_decile_impacts"):
            ...
            for decile_num in range(1, 11):
                with logfire.span("calculate_decile", decile=decile_num):
                    di = PEDecileImpact(
        ...
        with logfire.span("store_decile_impacts", count=len(decile_results)):
        ...
        with logfire.span("calculate_program_statistics", is_uk=is_uk):
            ...
            if is_uk:
                ...
                PEProgrammeStats.model_rebuild(
                    _types_namespace={"Simulation": PESimClass}
                )
                ...
                for prog_name, prog_info in programmes.items():
                    with logfire.span("calculate_program_stat", program=prog_name):
                        try:
                            ps = PEProgrammeStats(
                                baseline_simulation=pe_baseline_sim,
                                reform_simulation=pe_reform_sim,
                            ...
                            console.print(
                                f"    Skipping {prog_name}: variable not found ({e})"
                            )
            else:
                from policyengine.core import Simulation as PESimClass
        ...
        with logfire.span("mark_report_complete"):
...
def process_pending_work():
    ...
    try:
        with logfire.span("process_pending_work"):
            # Process pending simulations
            with logfire.span("fetch_pending_simulations"):
            ...
            if pending_sims:
                console.print(
                    f"[blue]Found {len(pending_sims)} pending simulations[/blue]"
                )
                logfire.info("Found pending simulations", count=len(pending_sims))
            ...
            with logfire.span("fetch_pending_reports"):
            ...
            if pending_reports:
                console.print(
                    f"[blue]Found {len(pending_reports)} pending reports[/blue]"
                )
                logfire.info("Found pending reports", count=len(pending_reports))
            ...
            for report in pending_reports:
                try:
                    ...
                    logfire.error(
                        "Report failed", report_id=str(report.id), error=str(e)
                    )
